...
def create_map_fig(filename):
    ...
    for n, spec in enumerate(run):
        if spec.ms_level == 1:
            ...
            all_mz += list(spec.mz)
            all_i += list(spec.i)
            all_rt += len(spec.mz) * [rt]
            all_scan += len(spec.mz) * [spec.ID]
    ...
    cvs = ds.Canvas(plot_width=number_spectra, plot_height=4000)
    #agg = cvs.points(df,'rt','mz', agg=ds.sum("i"))
    agg = cvs.points(df,'scan','mz', agg=ds.sum("i"))
    zero_mask = agg.values == 0
    agg.values = np.log10(agg.values, where=np.logical_not(zero_mask))
    fig = px.imshow(agg, origin='lower', labels={'color':'Log10(count)'}, color_continuous_scale="Hot")
    fig.update_traces(hoverongaps=False)
    fig.update_layout(coloraxis_colorbar=dict(title='Abundance', tickprefix='1.e'))

    return fig

@app.callback([Output('tic-plot', 'children'), Output('map-plot', 'children'), Output('download-link', 'children')],
              [Input('usi', 'value')])
def draw_file(usi):
    ...
    if "MSV" in usi_splits[1]:
        ...
        return [dcc.Graph(figure=fig), dcc.Graph(figure=map_fig), ftp_link]
